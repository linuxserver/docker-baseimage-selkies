...
- **Phase 1.5 (dev scope): DONE 2026-08-28** — pushed `docker.io/dgilli/selkies-rhel9:latest` (first push manifest `sha256:46246466…` = c7; **re-pushed same day with R1 c8 = manifest `sha256:b70d42e3…`, current**); verified pull-by-digest + cold-boot smoke (web 200, ws 101, wallpaper); NRP mapping `deploy/nrp-selkies-rhel9.yaml`; gates closed F28 (no securityContext in NRP templates — rootful OK) + F30 (Docker Hub dev, tag ceremony deferred) + F55 (docker default seccomp allows ptrace ≥4.8). Production merge (their @@template@@ system + imagePullSecret + SLU registry/tags) = later work item
