...
- **Phase 1.5 (production artifact): DONE 2026-08-28** — `deploy/nrp/selkies-rhel9.yaml.template` (drop-in NRP placeholder template — our repo owns it per user; slu-nrp-k8s-vm = earlier attempt, reference only); fixes 3 breaking mismatches (port 3000, USERNAME/PASSWORD auth, encoder omitted → pixelflux H.264, F57); render + cross-check ALL PASS. **Cluster-side E2E pending** (needs NRP kube access: pull secret + PSA check + template swap + run)
