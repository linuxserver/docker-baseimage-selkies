...
- ✅ **Manual verification PASSED (user 2026-08-28: "It all looks great")**: browser → `abc`/secret → GNOME desktop + SLU wallpaper + H.264 stream + dashboard Install → FileZilla (R1). **Phase 1.5 production COMPLETE end-to-end** (image → private registry → template → cluster → browser)
- ✅ **Torn down by user post-verification**; secrets remain in `slu-researchtechnologies-dgilli`: `selkies-password` (temp — delete when unneeded), `dockerhub-dgilli` (pull secret — keep; `apply-nrp-e2e.sh` refreshes it on next run)
