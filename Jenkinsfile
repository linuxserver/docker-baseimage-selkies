pipeline {
  ...
  stages {
    ...
    // If this is a debiantrixie build use live docker endpoints
    ...
    stage('Update-Templates') {
      ...
      steps {
        sh '''#! /bin/bash
              ...
              # ${TEMPDIR}/docker-${CONTAINER_NAME}: Cloned branch debiantrixie of ${LS_USER}/${LS_REPO} for running the jenkins builder on
              # ${TEMPDIR}/repo/${LS_REPO}: Cloned branch debiantrixie of ${LS_USER}/${LS_REPO} for commiting various templated file changes and pushing back to Github
              # ${TEMPDIR}/docs/docker-documentation: Cloned docs repo for pushing docs updates to Github
              # ${TEMPDIR}/unraid/docker-templates: Cloned docker-templates repo to check for logos
              # ${TEMPDIR}/unraid/templates: Cloned templates repo for commiting unraid template changes and pushing back to Github
              git clone --branch debiantrixie --depth 1 https://github.com/${LS_USER}/${LS_REPO}.git ${TEMPDIR}/docker-${CONTAINER_NAME}
              ...
              if [[ "$(md5sum Jenkinsfile | awk '{ print $1 }')" != "$(md5sum ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile | awk '{ print $1 }')" ]]; then
                ...
                git checkout -f debiantrixie
                cp ${TEMPDIR}/docker-${CONTAINER_NAME}/Jenkinsfile ${TEMPDIR}/repo/${LS_REPO}/
                git add Jenkinsfile
                git commit -m 'Bot Updating Templated Files'
                git pull https://LinuxServer-CI:${GITHUB_TOKEN}@github.com/${LS_USER}/${LS_REPO}.git debiantrixie
                git push https://LinuxServer-CI:${GITHUB_TOKEN}@github.com/${LS_USER}/${LS_REPO}.git debiantrixie
    ...
    // If this is a debiantrixie build check the S6 service file perms
      ...
      steps {
        sh '''#! /bin/bash
              ...
              if [ "${NEW_PACKAGE_TAG}" != "${PACKAGE_TAG}" ]; then
                ...
                git --git-dir ${TEMPDIR}/${LS_REPO}/.git checkout -f debiantrixie
      ...
      steps {
        sh '''#! /bin/bash
              ...
              curl -H "Authorization: token ${GITHUB_TOKEN}" -X POST https://api.github.com/repos/${LS_USER}/${LS_REPO}/git/tags \
                -d '{"tag":"'${META_TAG}'",\
                  "object": "'${COMMIT_SHA}'",\
                  "message": "Tagging Release '${EXT_RELEASE_CLEAN}'-ls'${LS_TAG_NUMBER}' to debiantrixie",\
                  "type": "commit",\
              ...
              jq -n \
                ...
                --arg target_commitish "debiantrixie" \
    ...
    stage('Github-Release-Branch-Protection') {
      ...
      steps {
        echo "Setting up protection for release branch debiantrixie"
        sh '''#! /bin/bash
          curl -H "Authorization: token ${GITHUB_TOKEN}" -X PUT https://api.github.com/repos/${LS_USER}/${LS_REPO}/branches/debiantrixie/protection \
          -d $(jq -c .  << EOF
            {
