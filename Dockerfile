...
RUN \
  ...
  gcc -m32 -shared -fPIC -ldl \
    ...
    input_interposer.c && \
  echo "**** link legacy joystick interposer names ****" && \
  ln -s \
    selkies_input_interposer.so \
    /buildout/usr/lib/selkies_joystick_interposer.so && \
  ln -s \
    selkies_input_interposer_32.so \
    /buildout/usr/lib/selkies_joystick_interposer_32.so && \
...
RUN \
  ...
  python3 \
    ...
    /lsiopy && \
  pip install \
    https://github.com/selkies-project/pixelflux/releases/download/${PIXELFLUX_RELEASE}/pixelflux-${PIXELFLUX_RELEASE}-cp313-cp313-manylinux_2_28_x86_64.whl \
    https://github.com/selkies-project/pcmflux/releases/download/${PCMFLUX_RELEASE}/pcmflux-${PCMFLUX_RELEASE}-cp313-cp313-manylinux_2_28_x86_64.whl \
...
COPY --from=ghcr.io/linuxserver/selkies-layers:amd64-debiantrixie-xvfb / /
COPY --from=ghcr.io/linuxserver/selkies-layers:amd64-debiantrixie-wtype / /
COPY --from=ghcr.io/linuxserver/selkies-layers:amd64-debiantrixie-selkies-desktop / /
COPY --from=ghcr.io/linuxserver/selkies-layers:amd64-debiantrixie-labwc / /
