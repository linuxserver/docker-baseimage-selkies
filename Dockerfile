...
RUN \
  ...
  echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker.asc] https://download.docker.com/linux/ubuntu resolute stable" > /etc/apt/sources.list.d/docker.list && \
  ...
  PCMFLUX_COMMIT=$(curl -sX GET "https://api.github.com/repos/selkies-project/pcmflux/commits/main" \
    | jq -r '.sha' | cut -c1-7) && \
  PIXELFLUX_VERSION=$(curl -s "https://raw.githubusercontent.com/selkies-project/pixelflux/${PIXELFLUX_COMMIT}/pixelflux/Cargo.toml" \
    | awk -F'"' '/^version = /{print $2;exit}') && \
  PCMFLUX_VERSION=$(curl -s "https://raw.githubusercontent.com/selkies-project/pcmflux/${PCMFLUX_COMMIT}/pcmflux/Cargo.toml" \
    | awk -F'"' '/^version = /{print $2;exit}') && \
  pip install \
