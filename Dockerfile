...
RUN \
  ...
  git clone \
    ...
    /src && \
  cd /src && \
  git checkout -f 988c15feeb30ea997d720dc832c5b5e621330109
...
FROM ghcr.io/linuxserver/baseimage-ubuntu:noble AS wtype
...
FROM ghcr.io/linuxserver/baseimage-ubuntu:noble AS wlroots
...
RUN \
  ...
  mkdir /debout && \
  cd /tmp && \
  apt-get source libwlroots12t64 && \
  cd wlroots-0.17* && \
  cp \
    /pass.c \
  ...
  cp \
    ../libwlroots12t64_0.17*.deb \
    /debout/wl.deb

FROM ghcr.io/linuxserver/baseimage-ubuntu:noble AS selkies-desktop
...
RUN \
  ...
  curl -o \
    ...
    "https://github.com/selkies-project/selkies/archive/988c15feeb30ea997d720dc832c5b5e621330109.tar.gz" && \
  cd /tmp && \
  ...
  echo "**** cleanup ****" && \
  pip uninstall -y pixelflux && \
  pip install pixelflux==1.6.2 && \
